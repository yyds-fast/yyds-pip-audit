    ...
    try:
        ...
        if temp_path is not None:
            try:
                ...
                pass
        raise click.ClickException(
            f"Failed to write output file {target}: {exc}"
        ) from exc
...
@click.option(
    ...
    help="Save dependency output to specified file (e.g. requirements.txt)",
)
@click.option(
    "-f",
    "--format",
    "output_format",
    type=click.Choice(["text", "requirements", "json"]),
    default=None,
    help="Format of the dependency output: text (terminal table), requirements (standard dependencies), json (JSON data)",
)
@click.option(
    "-e",
    "--exclude",
    multiple=True,
    help="Extra directory names to exclude (can be specified multiple times)",
)
@click.option(
    "--source-root",
    multiple=True,
    help="Project import root used to identify local packages (e.g. src)",
)
@click.option(
    "-c",
    "--check",
    type=click.Path(exists=True, dir_okay=False),
    help="Audit and compare against specified requirements file to analyze missing and unused dependencies",
)
@click.option(
    "--fail-on",
    type=click.Choice(["none", "missing", "unused", "any"]),
    default=None,
    help="Return exit code 1 when selected requirement issues are found",
)
@click.option(
...
def main(
    ...
    if not isinstance(output_format, str) or output_format not in {
        ...
        raise click.ClickException(
            f"Unsupported output format in configuration: {output_format}"
        )
    ...
    if not isinstance(configured_evaluate_markers, bool):
        raise click.ClickException(
            "Configuration value evaluate_markers must be a boolean"
        )
    ...
    configured_source_roots = _config_string_list(
        config, "source_roots", "source-roots"
    )
    ...
    if check:
        ...
        audited_pypi_names = {
            normalize_package_name(item["pypi_name"]) for item in results
        }
    ...
    if output_format == "json":
        ...
        if output:
            ...
            console.print(
                f"[green]JSON audit results successfully saved to {output}[/green]"
            )
        else:
        ...
        if output:
            write_text_atomic(output, formatted_content)
            console.print(
                f"[green]Dependencies list successfully exported to {output}[/green]"
            )
        else:
        ...
        if not results:
            console.print(
                "[yellow]No third-party dependency imports detected.[/yellow]"
            )
        else:
        ...
        if check:
            ...
            if missing_in_reqs:
                console.print(
                    "[bold red]❌ Missing Dependencies (imported in code but not registered in requirements):[/bold red]"
                )
                for item in missing_in_reqs:
                    ver_suffix = (
                        f" (local version: {item['version']})"
                        if item["version"]
                        else ""
                    )
                    display_imp = format_display_imports(item["import_name"])
                    console.print(
                        f"  • [red]{item['pypi_name']}[/red] [dim](introduced by import {display_imp}){ver_suffix}[/dim]"
                    )
            else:
                console.print(
                    "[green]✔ No missing dependencies found (all imports are registered in requirements)[/green]"
                )
